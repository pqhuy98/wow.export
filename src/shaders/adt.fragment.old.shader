...
void main() {
	...
	float a0 = texture(pt_blend1, mod(vTextureCoord, 1.0)).r;
	float a1 = texture(pt_blend2, mod(vTextureCoord, 1.0)).r;
	float a2 = texture(pt_blend3, mod(vTextureCoord, 1.0)).r;

	vec3 t0 = texture(pt_layer0, tc0).rgb;
	vec3 t1 = texture(pt_layer1, tc1).rgb;
	vec3 t2 = texture(pt_layer2, tc2).rgb;
	vec3 t3 = texture(pt_layer3, tc3).rgb;

	float base = 1.0 - (a0 + a1 + a2);
	vec3 color = t0 * base + t1 * a0 + t2 * a1 + t3 * a2;
	fragColor = vec4(color * vVertexColor.rgb * 2.0, 1.0);
}

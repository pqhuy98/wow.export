...
const _doPreload = async () => {
	try {
		...
		for (const [fileDataID, fileName] of preloadedIdLookup.entries()) {
			...
			switch (ext) {
				case '.blp':
					preloadTextures.push(fileDataID);
					break;
				case '.ogg':
				case '.mp3':
				case '.unk_sound':
					preloadSounds.push(fileDataID);
					break;
				case '.avi':
					preloadVideos.push(fileDataID);
					break;
				case '.txt':
				case '.lua':
				case '.xml':
				...
				case '.toc':
				case '.xsd':
					preloadText.push(fileDataID);
					break;
				case '.m3':
					if (core.view.config.modelsShowM3)
						preloadModels.push(fileDataID);
					break;
				case '.m2':
					...
					break;
				case '.wmo':
					if (core.view.config.modelsShowWMO && !fileName.match(constants.LISTFILE_MODEL_FILTER))
						preloadModels.push(fileDataID);
					break;
				default:
					break;
